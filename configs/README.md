...
当前比赛预警规则统一读取 [`warning_rules.yaml`](warning_rules.yaml)。雷达TTC、视觉路径与tau、GPS修正、IMU横向失稳、数据时效和下降保持参数都在该文件中，并带有版本与校准状态。

[`risk_params.yaml`](risk_params.yaml) 只供旧加权`RiskModel`和兼容测试使用，不得用于解释当前规则预警结果。
