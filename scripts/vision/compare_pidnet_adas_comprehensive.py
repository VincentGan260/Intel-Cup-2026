"""PIDNet-S vs road-adas 综合对比测试脚本。
...
运行：
    python scripts/vision/compare_pidnet_adas_comprehensive.py
"""
...
def evaluate_on_dataset(model_xml: Path, model_name: str, dataset: dict, num_samples: int, input_h: int, input_w: int, road_class_index: int) -> dict:
    ...
    for img_path, img_name in images:
        ...
        miou = compute_miou(pred_mask, gt_mask, 2, ignore_value)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PIDNet-S vs road-adas 综合对比测试")
    ...
    print("=" * 80)
    print("PIDNet-S vs road-adas 综合对比测试")
    print("=" * 80)
    print(f"测试设备: CPU")
    print(f"精度: FP32")
    print(f"统一输入尺寸: 512x896")
    print(f"评测样本: {num_samples} 张/数据集")
    print(f"可视化数量: {args.vis_count} 张/数据集")
    print(f"注: road-adas 输入 512x896, PIDNet-S 输入 1024x1024 (模型固定)")
    print("-" * 80)
    ...
    print("\n【速度基准测试】")
    print("-" * 80)
    print(f"{'模型':<12} {'均值(ms)':>10} {'中位(ms)':>10} {'p95(ms)':>10} {'标准差(ms)':>12} {'FPS':>8} {'CPU(%)':>10} {'内存(MB)':>12}")
    print("-" * 80)
    
    speed_results = {}
    for m in models:
        if not m["xml"].exists():
            print(f"{m['name']:<12} {'模型不存在':>65}")
            continue
        
        result = bench_single_model(m["xml"], "CPU", 20, 200, m["input_hw"][0], m["input_hw"][1], m["name"])
        if result["success"]:
            print(f"{m['name']:<12} {result['mean_ms']:>10} {result['median_ms']:>10} {result['p95_ms']:>10} "
                  f"{result['std_ms']:>12} {result['fps']:>8} {result['mean_cpu_pct']:>10} {result['memory_mb']:>12}")
    ...
    for ds in datasets:
        print(f"\n数据集: {ds['name']}")
        print("-" * 80)
        print(f"{'模型':<12} {'图片数':>8} {'road IoU(%)':>14} {'mIoU(%)':>12} {'像素准确率(%)':>16} {'延迟(ms)':>10} {'FPS':>8} {'CPU(%)':>10}")
        print("-" * 80)
        ...
        for m in models:
            if not m["xml"].exists():
                ...
                print(f"{m['name']:<12} {0:>8} {0.0:>14} {0.0:>12} {0.0:>16} {0.0:>10} {0.0:>8} {0.0:>10}")
                continue
            ...
            print(f"{m['name']:<12} {result['count']:>8} {result['iou']:>14} {result['miou']:>12} "
                  f"{result['pixel_acc']:>16} {result['mean_latency_ms']:>10} {result['fps']:>8} {result['mean_cpu_pct']:>10}")
    ...
    if speed_results:
        print("\n速度对比:")
        print(f"{'指标':<20} {'road-adas':>15} {'PIDNet-S':>15} {'差异':>15}")
        print("-" * 65)
        ...
        if road_adas and pidnet:
            ...
            for name, key in metrics:
                ...
                print(f"{name:<20} {ra_val:>15} {pid_val:>15} {f'{diff:.2f} ({diff_pct:.1f}%)':>15}")
    
    if accuracy_results:
        print("\n精度对比 (road IoU):")
        print(f"{'数据集':<12} {'road-adas':>15} {'PIDNet-S':>15} {'差异':>15}")
        print("-" * 57)
        ...
        for ds_name, ds_results in accuracy_results.items():
            ...
            if road_adas_acc and pidnet_acc:
                ...
                print(f"{ds_name:<12} {road_adas_acc.get('iou', 0):>15} {pidnet_acc.get('iou', 0):>15} {f'{iou_diff:+.2f}%':>15}")
    
    print("\n稳定性分析:")
    print(f"{'模型':<12} {'延迟标准差(ms)':>20} {'延迟波动(%)':>20} {'CPU波动(%)':>20}")
    print("-" * 72)
    ...
    for m in models:
        ...
        if speed_info.get("success"):
            ...
            print(f"{m['name']:<12} {lat_std:>20} {lat_fluct:>19.1f}% {cpu_fluct:>19.1f}%")
    ...
    results = {
        ...
        "device": "CPU",
        "precision": "FP32",
        "input_size": {"road-adas": "512x896", "PIDNet-S": "1024x1024"},
        "num_samples": num_samples,
        "speed": speed_results,
        "accuracy": clean_accuracy
    }
    
    out_path = out_dir / "pidnet_vs_adas_comprehensive.json"
    out_path.write_text(json.dumps(results, ensure_ascii=False, indent=2), encoding="utf-8")
    
    csv_path = out_dir / "pidnet_vs_adas_comprehensive.csv"
    with open(csv_path, "w", newline="", encoding="utf-8-sig") as f:
        writer = csv.writer(f)
        writer.writerow(["数据集", "模型", "图片数", "road IoU(%)", "mIoU(%)", 
                         "像素准确率(%)", "平均延迟(ms)", "FPS", "CPU占用(%)"])
    ...
    print(f"\n结果已保存至: {out_dir}")
    print(f"  - pidnet_vs_adas_comprehensive.json")
    print(f"  - pidnet_vs_adas_comprehensive.csv")
