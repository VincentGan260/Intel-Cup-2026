...
对比 base (COCO) 和 v2 (微调) 两个模型在 FP32 和 FP16 精度下的检测精度。
使用所有数据集（BDD + IDD + DAWN），统一使用 GPU。

指标：mAP@0.5、mAP@0.5:0.95、小/中/大目标 AP、recall@0.5、两轮车 recall。
"""
...
MODEL_CONFIG = {
    "base": {
        "fp32": "models/yolo26n_base_openvino_model/yolo26n.xml",
        "fp16": "models/yolo26n_base_fp16_openvino_model/yolo26n.xml",
    },
    "v2": {
        "fp32": "models/yolo26n_v2_openvino_model/best.xml",
        "fp16": "models/yolo26n_v2_fp16_openvino_model/best.xml",
    },
}
...
def evaluate_model(model_name: str, precision: str, device: str = "GPU", limit: int = 0):
    import torch
    ...
    model_path = resolve(MODEL_CONFIG[model_name][precision])
    print(f"\n{'='*78}")
    print(f"评估模型: {model_name} ({precision})")
    ...
    for i, r in enumerate(rows, 1):
        ...
        pb, pc = detector.predict(img_path)
    ...
    result = {"model": model_name, "precision": precision, "device": device,
              "total_images": len(rows), "elapsed_time": round(elapsed_total, 2), "sources": {}}
    ...
    for ds in ["bdd", "idd", "dawn"]:
        ...
        if ds == "bdd":
            ...
            pu = precision_over([R[i] for i in idx_unbiased])

            idx_night = [i for i, rec in enumerate(R) if rec["row"].get("timeofday") == "night"]
            mn = map_over(P, T, idx_night)
            rn = recall_over([R[i] for i in idx_night])
            pn = precision_over([R[i] for i in idx_night])

            print(f"  ├ 无偏总体({len(idx_unbiased)}张): mAP@0.5={mu['map50']} recall={ru}% precision={pu}%")
            print(f"  └ 夜间({len(idx_night)}张): mAP@0.5={mn['map50']} recall={rn}% precision={pn}%")
            result["sources"][ds]["unbiased"] = {"n": len(idx_unbiased), "map50": mu["map50"], "recall": ru, "precision": pu}
            result["sources"][ds]["night"] = {"n": len(idx_night), "map50": mn["map50"], "recall": rn, "precision": pn}
...
def print_summary(all_results):
    print("\n" + "=" * 78)
    print("YOLO26 模型精度对比汇总")
    print("=" * 78)

    print("\n1. mAP@0.5 对比:")
    print("-" * 78)
    print(f"| {'模型':<8} | {'精度':<6} | {'BDD':>8} | {'IDD':>8} | {'DAWN':>8} |")
    print("-" * 78)
    for r in all_results:
        bdd = r["sources"].get("bdd", {}).get("map50", "-")
        idd = r["sources"].get("idd", {}).get("map50", "-")
        dawn = r["sources"].get("dawn", {}).get("map50", "-")
        print(f"| {r['model']:<8} | {r['precision']:<6} | {bdd:>8} | {idd:>8} | {dawn:>8} |")
    print("-" * 78)
    ...
    for ds in ["bdd", "idd", "dawn"]:
        print(f"\n[{ds}]")
        print(f"| {'模型':<8} | {'精度':<6} | {'小目标':>8} | {'中目标':>8} | {'大目标':>8} |")
        print("-" * 78)
        for r in all_results:
            ...
            print(f"| {r['model']:<8} | {r['precision']:<6} | {sm:>8} | {md:>8} | {lg:>8} |")


def main():
    ap = argparse.ArgumentParser(description="YOLO26 模型精度对比评测")
    ap.add_argument("--device", default="GPU", help="推理设备（GPU/CPU/NPU）")
    ap.add_argument("--limit", type=int, default=0, help=">0 时每源最多评这么多张")
    ap.add_argument("--output", default="runs/accuracy_eval/compare_yolo26_precision.json", help="结果输出路径")
    args = ap.parse_args()
