...
- `radar.jsonl`、`imu.jsonl`、`gps.jsonl`：各路原始解析数据。
...
同步以相机帧为时间轴，默认匹配最近的雷达（100 ms）、IMU（50 ms）和 GPS（1000 ms）。超过阈值的数据保留但标记 `valid=false`，便于重新调整阈值而无需重新采集。

检查 `quality_report.json`：四路记录数不能为零、时间戳不能倒退、图片不能缺失；重点查看 `fusion.valid_ratio` 和 `max_abs_delta_ms`。
