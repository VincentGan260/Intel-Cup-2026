...
>
> 当前测试结论：相机、LD2451、目标检测、语义分割、Dashboard和统一Recorder已跑通；GPS当前无法搜星，已降级为可选模态，失效时记录`gps.valid=false`且不阻塞相机/视觉/雷达数据采集。现有数据只作为管线测试集。完成本文 P0 并通过60秒短采验收后，才能开始正式长时间采集。
...
- [x] GPS降级为可选模态；正式`--record`默认不等待GPS，只有显式`--wait-gps`才要求定位。
...
- [ ] 尚需DK2500真机完成60秒室外雷达/视觉短采、雷达—相机标定和异构设备基准；GPS修复后再补GPS短采，这些项目不能在无硬件开发机上代验收。
...
- [x] GPS当前失效时不阻塞录制；正式数据中以`gps.valid=false`记录，训练时使用模态mask。
- [ ] 录制60秒并运行自动质量检查。
- [ ] 图片数与JSONL样本数一致，无缺图。
